...
produced by data_preprocessing_feature_exports.py, using Optuna hyperparameter
tuning with stratified group k-fold cross-validation (k=5), and evaluates on
the primary held-out test set.

Pipeline
--------
1. Load the MRF train and test splits from disk.
2. Separate feature columns from metadata (subject_id, group, labels, outcome-
...
Usage::
    ...
    python model_mrf_evaluation.py
    python model_mrf_evaluation.py --n_iter 100 --seed 42 --n_jobs 4
"""
...
def train_mrf_model(
    ...
    """
    Tune and fit a CatBoost model on the MRF training set.
    ...
    logger.info(
        f"Training CatBoost on MRF features: "
...
def evaluate_on_test(
    ...
    logger.info(
        f"MRF CatBoost  AUC = {auc:.3f}  95% CI [{ci_low:.3f}, {ci_high:.3f}]"
    )
    return {
        "auc": auc,
        "ci_low": ci_low,
...
def plot_roc(result: dict, output_path: str) -> None:
    ...
    RocCurveDisplay.from_predictions(
        ...
        result["y_score"],
        ax=ax,
        name=f"MRF CatBoost (AUC = {result['auc']:.3f})",
        color="darkorange",
        plot_chance_level=True,
    )
    ax.set_title(f"MRF — ROC Curve  (primary test set, n = {n_pairs} pairs)")
...
def save_results(
    ...
    metrics_df.to_csv(f"{output_dir}/mrf_evaluation.csv", index=False)
    importance_df.to_csv(f"{output_dir}/mrf_feature_importance.csv", index=False)
...
def run(
    train_path: str = "data/adni_mrf_features_train.csv",
    test_path: str = "data/adni_mrf_features_test.csv",
    ...
    study, best_model, inner_splits = train_mrf_model(
        train_df, test_df,
    ...
    plot_roc(result, output_path=f"{plots_dir}/mrf_roc.pdf")
    save_results(result, study, importance_df, output_dir)
    joblib.dump(
        {"model": best_model, "study": study, "feature_cols": feature_cols, "result": result},
        f"{output_dir}/mrf_model.joblib",
    )
    logger.info(f"Model saved to {output_dir}/mrf_model.joblib")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", default="data/adni_mrf_features_train.csv")
    parser.add_argument("--test", default="data/adni_mrf_features_test.csv")
